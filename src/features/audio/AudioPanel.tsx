...
export function AudioPanel({ onSceneChange, locked }: Props) {
  ...
  useEffect(() => {
    function tick() {
      ...
      if (match && match.sceneId !== lastSceneRef.current) {
        ...
        if (onSceneChange) onSceneChange(match.sceneId)
        else setActiveScene(match.sceneId)
      }
  ...
  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return
    setError('')
    if (!file.type.startsWith('audio/')) { setError('Solo se permiten archivos de audio (MP3, WAV, OGG).'); return }
    if (file.size > MAX_SIZE_MB * 1024 * 1024) { setError(`El archivo supera ${MAX_SIZE_MB}MB`); return }
    ...
    <div className="border-t border-borde bg-surface-1 text-blanco-calido p-3 shrink-0">
      ...
      <div className="flex items-start gap-4">
        <div className="flex flex-col gap-2 shrink-0 w-40">
          ...
          <label className="cursor-pointer px-3 py-2 border border-dashed border-borde rounded-lg text-xs text-gris hover:border-dorado/50 hover:text-dorado transition-colors text-center">
          ...
          <div className="flex gap-1">
            ...
            >{isPlaying ? '⏸' : '▶'}</button>
            <button
              onClick={() => { if (!audioRef.current) return; audioRef.current.pause(); audioRef.current.currentTime = 0; setCurrentTime(0); lastSceneRef.current = null; prevCheckMsRef.current = 0 }}
              disabled={!fileName}
          ...
          {duration > 0 && (
            <div ref={timelineRef} className="relative h-9 bg-surface-1 rounded-lg border border-borde/40 overflow-hidden select-none">
              ...
              {scenes.map((scene, i) => {
                ...
                return (
                  <div key={scene.id}>
                    <div className="absolute top-0 bottom-0 flex items-center cursor-pointer hover:bg-white/5 transition-colors"
                      style={{ left: `${(i / scenes.length) * 100}%`, width: `${100 / scenes.length}%` }}
                      title={`Click para marcar "${scene.name}"`} onClick={e => handleTimelineClick(e, scene.id)}>
                      <span className="text-[9px] text-gris/40 px-1.5 truncate">{i + 1}</span>
                    </div>
                    {pct !== null && (
                      <div className="absolute top-0 bottom-0 w-0.5 bg-dorado cursor-ew-resize group z-10"
                        style={{ left: `${pct}%` }} onMouseDown={() => setDraggingMarker(scene.id)}
                        title={`${scene.name}: ${fmt(ms! / 1000)}`}>
                        <div className="absolute -top-0 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full bg-dorado" />
                        <div className="absolute top-3 left-2 text-[8px] text-dorado whitespace-nowrap opacity-0 group-hover:opacity-100 bg-negro/80 px-1 rounded">{scene.name}</div>
                      </div>
                    )}
                  </div>
                )
              ...
              {draggingMarker && (
                <div className="absolute inset-0 cursor-ew-resize z-20"
                  onMouseMove={e => { if (!timelineRef.current || duration === 0) return; const rect = timelineRef.current.getBoundingClientRect(); const t = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width)); setSceneMarker(draggingMarker, Math.round(t * duration * 1000)) }}
                  onMouseUp={() => setDraggingMarker(null)} onMouseLeave={() => setDraggingMarker(null)} />
              )}
            </div>
          )}
          {duration > 0 && <p className="text-[10px] text-gris/40">Click en la barra para marcar cuándo empieza cada escena · Arrastrá el pin para ajustar</p>}
        </div>
      </div>
    </div>
  )
